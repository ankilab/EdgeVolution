...
def read_inference_time(board, save_dir=None):
            ...
            while len(measured_values) < 1:
                ...
                try:
                    ...
                    if line != b'':
                        ...
                        elif "opcode" in str(line):    
                            number = re.findall(r'\d+', str(line))[0]
                            print(f"{number} with op: " + operators_dict[int(number)])
                            ops.append(operators_dict[int(number)])
                        else:
                            print(str(line))
    ...
    with open("test/c_ops.json", "r") as f:
        data = json.load(f)    
    with open("test/c_ops.json", "w") as f:
        data.append(ops)
        json.dump(data, f)
